<template>
  <div class="tab-body">
    <form @submit.prevent="signUp" novalidate>
      <div class="input-filds input-filds-registration">
        <p class="input-group">
          <phone-mask-input
            type="text "
        ...
        <p class="input-group">
          ...
          />
          <small class="invalid" v-if="!$v.first_name.minLength"
            >{{ $t("auth.valid_name") }}
        ...
        <p class="input-group">
          ...
          />
          <small class="invalid" v-if="!$v.last_name.minLength"
        ...
        <small class="invalid" v-if="!$v.password2.sameAsPassword">
      ...
      <div class="remember remember-d">
        <div class="remember__me remember-d__me">
          <input
            type="checkbox"
            class="check-me"
            id="check-registr"
            v-model="handling_data"
            @change="$v.handling_data.$touch()"
            :class="{ invalid: $v.handling_data.$error }"
          />
          <label for="check-registr"
            >{{ $t("auth.agree") }}
            <a
              class="link-personal"
              :href="$router.resolve({ name: 'personal' }).href"
              >{{ $t("auth.personal_data") }}</a
            ></label
          >
        </div>
      </div>
      <div class="btn-enter">
        ...
        <button type="submit" class="logIn" :disabled="submitRegister" v-else>
          {{ $t("auth.button_text") }}
        </button>
      </div>
    </form>
  ...
  computed: {
    ...
    submitRegister() {
      return (
        ...
        this.$v.handling_data.$invalid
      );
    },
  },
  validations: {
    phone: { required },
    first_name: { minLength: minLength(2) },
    last_name: { minLength: minLength(2) },
    ...
    signUp() {
      this.$api.auth
        .signUp({
          ...
          handling_data: this.handling_data,
        })
        .then((response) => {
